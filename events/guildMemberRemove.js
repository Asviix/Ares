module.exports = (client, member) => {
  ...
  client.db.query(`SELECT \`language\`, \`logs_id\`, \`logs_int\` FROM \`servers\` WHERE \`id\` = ${channel.guild.id}`, async (err, result) => {
    ...
    if (logs_int[lang.LOGS_MEMBERS_MODIFIER + 1] == 1) {
      const timestamp = `${moment(new Date()).format("YYYY-MM-DD HH:mm:ss:ms")}`
      let embed = new MessageEmbed()
        .setTitle(eval(lang.MEMBER_LEFT))
        .setColor(`#3B95EA`)
        .addFields(
          {name: lang.MEMBER_NAME, value: member.user.tag, inline: true},
          {name: lang.MEMBER_LEFT_DATE, value: timestamp, inline: true},
        )
